...
if [[ "$set_data_yn" ==  "y" || "$set_data_yn" ==  "Y" ]]; then
    ...
    read -p "Scaler ($scaler): " user_input
    if [ -n "$user_input" ]; then
        scaler="$user_input"
    fi
    read -p "Test Size ($test_size): " user_input
    if [ -n "$user_input" ]; then
        test_size="$user_input"
    fi
    read -p "Random State ($random_state): " user_input
    if [ -n "$user_input" ]; then
        random_state="$user_input"
    fi
fi
